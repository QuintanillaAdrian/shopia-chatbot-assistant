...
$result = null;
if ( class_exists( 'Chatbot_Assistant_Provision' ) && method_exists( 'Chatbot_Assistant_Provision', 'generate_wc_keys_direct' ) ) {
    $result = Chatbot_Assistant_Provision::generate_wc_keys_direct();
} else {
    // Intentar cargar el archivo include si por alguna razón la clase no está presente.
    if ( file_exists( __DIR__ . '/includes/class-chatbot-assistant-provision.php' ) ) {
        require_once __DIR__ . '/includes/class-chatbot-assistant-provision.php';
    }
    if ( class_exists( 'Chatbot_Assistant_Provision' ) && method_exists( 'Chatbot_Assistant_Provision', 'generate_wc_keys_direct' ) ) {
        $result = Chatbot_Assistant_Provision::generate_wc_keys_direct();
    }
